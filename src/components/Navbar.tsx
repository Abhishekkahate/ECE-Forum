...
export const Navbar: React.FC<NavbarProps> = ({ onOpenGoogleAuth, onOpenMyPasses, onOpenAdmin }) => {
  ...
  const updatePassCount = () => {
    if (user?.email) setUserPassCount(passService.getUserPasses(user.email).length);
    else setUserPassCount(0);
  };
  ...
  useEffect(() => {
    updatePassCount();
    const h = () => updatePassCount();
    window.addEventListener('ece_passes_updated', h);
    return () => window.removeEventListener('ece_passes_updated', h);
  }, [user?.email]);
  ...
  return (
    <>
      ...
      >
        ...
        {mobileMenuOpen && (
          <div className="xl:hidden mx-3 mt-2 rounded-[22px] bg-[rgba(12,12,14,0.96)] backdrop-blur-3xl border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.55)] overflow-hidden">
            <div className="h-px bg-gradient-to-r from-transparent via-[#FF4A15]/40 to-transparent" />
            <div className="p-3 space-y-3 max-h-[72vh] overflow-y-auto">
              <div className="grid grid-cols-2 gap-2">
                {navLinks.map((link) => {
                  const isActive = activeLink === link.href;
                  return (
                    <a key={link.label} href={link.href} onClick={() => { soundFx.playClick(); setMobileMenuOpen(false); }} className={`px-3.5 py-3 rounded-xl border flex items-center justify-between text-sm font-medium transition-colors ${isActive ? 'bg-[#FF4A15] border-[#FF4A15] text-white font-bold' : 'bg-white/[0.04] border-white/[0.06] text-white/85 hover:bg-white/[0.07]'}`}>
                      <span className="uppercase text-[12.5px]">{link.label}</span><span className={`text-[11px] font-mono ${isActive ? 'text-white/70' : 'text-white/30'}`}>{link.num}</span>
                    </a>
                  );
                })}
              </div>
              <div className="grid grid-cols-2 gap-2 pt-3 border-t border-white/[0.07]">
                <button onClick={() => { setMobileMenuOpen(false); onOpenMyPasses(); }} className="py-3 rounded-xl bg-[#FF4A15]/10 border border-[#FF4A15]/20 text-[#FF4A15] font-mono text-xs font-bold flex items-center justify-center gap-2"><Ticket className="w-4 h-4" /> Passes ({userPassCount})</button>
                <button onClick={() => { setMobileMenuOpen(false); onOpenAdmin(); }} className="py-3 rounded-xl bg-white/[0.05] border border-white/[0.08] text-white/70 font-mono text-xs font-bold flex items-center justify-center gap-2"><Shield className="w-4 h-4" /> Admin</button>
              </div>
            </div>
          </div>
        )}
